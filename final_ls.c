...
int     ls_function(int Check, int aflag, int lflag, int hflag, int sflag, int rflag, int argc, char **argv);
...
///////////////////////////// End check options ////////////////////////////////
  ...
  ls_function(CheckOpt, aflag, lflag, hflag, sflag, rflag, argc, argv); // call ls function
  return 0;
}
// End Main
...
//////////////////////////////// Name variable /////////////////////////////////
      ...
      else if(size1 == size2 && DotCompare(str1, str2) > 0){ // size equal  -> a b c d...
          if(ptr == list->Head->NextNode){ // Insert before head
            NewNode->NextNode = ptr;
            NewNode->Prev = list->Head;
            list->Head->NextNode = NewNode;
...
/////////////////////////////////// -l option //////////////////////////////////
    case 'l':
              ...
              while(ptr != list->Tail){
                ...
                printf("\t%-2s %-2d",((DInfo*)ptr->Data)->date_str,((DInfo*)ptr->Data)->date_int[0]);
...
/////////////////////////////////// -h option //////////////////////////////////
    case 'h':
              ...
              while(ptr != list->Tail){
                ...
                printf("\t%s", ((DInfo*)ptr->Data)->userid);
                printf("\t%s", ((DInfo*)ptr->Data)->groupid);
                if((((DInfo*)ptr->Data)->filesize) > 1024) printf("\t%dK", (((DInfo*)ptr->Data)->filesize) / 1024);
                else  printf("\t%d", ((DInfo*)ptr->Data)->filesize);
                printf("\t%-2s %-2d",((DInfo*)ptr->Data)->date_str,((DInfo*)ptr->Data)->date_int[0]);
                printf("\t%02d:%02d",((DInfo*)ptr->Data)->time[0],((DInfo*)ptr->Data)->time[1]);
                printf("\t%s",((DInfo*)ptr->Data)->filename);
                printf("\n");
...
////////////////////////// Making temp /////////////////////////////////////////
  strcpy(Temp, Dirname);
////////////////////////// Check * input ///////////////////////////////////////
  if(strcmp(Dirname, check) == 0) strcpy(Dirname, ".");
//////////////////// Dir is current Dir ////////////////////////////////////////
            ...
            while((dir = readdir(dirp)) != NULL){ // Read directory
              ...
              if(!lstat(dir->d_name, &buf)){ // Get file buf
                if(!S_ISDIR(buf.st_mode)){ // Input is not Dir
                  ...
                  if((List_Insert(list, path, 'n')) == -1){ // List insert
                    printf("List insert error\n"); // List insert ERROR
                    return 0;
                  }
                }
